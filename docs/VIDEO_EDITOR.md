...
| **L2（本文档主体）** | 基础竖屏成片：裁段 + 烧字幕 + 品牌样式 + 配乐 → MP4 + SRT；文字稿式编辑 + 单轨 trim | **要做，且刻意做浅** |
...
```jsonc
{
  "source": { "asset_id": "uuid", "url": "https://.../proxy.mp4", "fps": 30 },
  ...
  "music": { "track_id": "calm-01", "enabled": true, "gain_db": -18 },
  "brand_ref": "brand_template_uuid",       // logo/色/字体来自品牌模板
  "target_language": "en"
}
```
...
1. Range 流式端点（本地文件，可播放/seek）+ 源视频 proxy 转码（格式归一）
   —— 保留本地存储，对象存储留到规模化（ADR-011 P1）
2. 多语 ASR（词级时间戳）→ 接进 worker 的 asset processor
   （1、2 相互独立，可并行起手）
3. clip-spec 契约 + 表迁移（加 render_*，顺手清死列 keyframes/subtitles）
   + 新旧模型协调（render_spec 替代/seed 旧 Clip.script）
   + Remotion 组件 + Node 渲染服务 + 队列触发 → 打通"spec→MP4+SRT"
4. 编辑器 UI：<Player> 预览 + 文字稿编辑（删句=剪段/非破坏性）+ 单轨 trim + 样式/标题/配乐面板
```
